---
title: "Burnout_Meta_Satisfaction"
author: "H.L. Glandorf"
date: '2022-07-08'
output: pdf_document
---
...
## Burnout Outcomes Meta analysis: Satisfaction
...
Creating separate data frames for different constructs (here: satisfaction)

```{r}
# satisfaction
s_dat <- dat %>% 
  filter(outcome=="satisfaction") %>% # filtering the data for satisfaction as an outcome only
  filter(!is.na(yi)) # filtering out missing values in fisher's z score
```

Three-level meta-analysis model (here: satisfaction)

Simple model
```{r}
mod_s <- rma.mv(yi = yi, # specify fisher's z
                V = vi, # specify variance
                slab = authors, # specify authors
                data = s_dat, # specify data set
...
summary(mod_s) # show summary of the multilevel meta model
convert_z2r(mod_s$b) # convert to correlation r
predict.intervals_mod_s <- predict(mod_s, digits = 2, level = .95) # determine  prediction intervals
print(predict.intervals_mod_s) # show prediction intervals
```
...
mod_s.within.anova <- anova(mod_s, mod_s.within)
mod_s.within.anova
mod_s.between.anova <- anova(mod_s, mod_s.between)
mod_s.between.anova 
# not significant on the between level, as such, variance on the within level seems to explain most heterogeneity
...
calculate_I2 <- function(s_dat, mod_s){
  W <- diag(1/s_dat$vi) # define weight matrix (gives more weight to studies with smaller variances)
  X <- model.matrix(mod_s) # design matrix with including predictors 
  ...
  I2_statistic <- 100 * sum(mod_s$sigma2) / (sum(mod_s$sigma2) + (mod_s$k-mod_s$p)/sum(diag(P))) # then calculate I2 based on defined matrices above
  return (I2_statistic)
}

I2 <- calculate_I2(s_dat, mod_s) # use function to calculate I2
I2
...
calculate_I2update <- function(s_dat, mod_s){
  W <- diag(1/s_dat$vi)
  X <- model.matrix(mod_s)
  P <- W - W %*% X %*% solve(t(X) %*% W %*% X) %*% t(X) %*% W
  I2update_statistic <- 100 * mod_s$sigma2 / (sum(mod_s$sigma2) + (mod_s$k-mod_s$p)/sum(diag(P))) # I2 update calculation no longer uses sum in the first numerator
  return (I2update_statistic)
}

I2update <- calculate_I2update(s_dat, mod_s)# use function to calculate I2 update 
I2update
...
# more heterogeneity at the within level, which reflects anova results from above
```
...
cooks.distance (mod_s, cluster=s_dat$st_id) # calculate cooks distance for study id clusters
plot(cooks.distance(mod_s, cluster=s_dat$st_id), type="o", pch=19)
## based on the 4/N rule, there are no outlier studies

# now, looking at the individual effects (level 2)
# calculate residuals
resid_d <- residuals(mod_s) %>%
...
png(filename = "ResidualsPlotSatisfaction.png", 
...
outliers_resid_d <- resid_d %>%
  cbind(s_dat$authors) %>%                # bind study names for reference
  subset(resid_d > 3.0 | resid_d < - 3.0) 

# Create new dataframe with residual outliers removed
# remove residual outliers 
s_datresidualoutliers_dropped <- s_dat %>%
  subset(subset = -3.0 < resid_d & resid_d < 3.0)
View(s_datresidualoutliers_dropped)

mod_s.residual <- rma.mv(yi, vi, 
                           data = s_datresidualoutliers_dropped, #use new dataframe here 
                           level = 95,
                           method = "REML", 
                           slab = authors, 
                           tdist = TRUE, 
                           random = list(~ 1 | st_id, 
                                         ~ 1 | eff_id)) 
summary(mod_s.residual) # summary of meta without outliers

# calculate cook's distance at individual effects (level 2) rather than studies (level 3)
cooks_d <- cooks.distance(mod_s) # note: this step can take time to execute

plot(cooks_d, type="p", pch=19)
png(filename = "CooksDistancePlotSatisfaction.png", 
    width = 800, height = 640, 
    pointsize = 12, res = 120)
plot(cooks_d, type="p", pch=19)
dev.off()
...
s_dat_cooksoutliers_dropped <- s_dat %>%
  cbind(cooks_d) %>%
  filter(cooks_d < 3.0*mean(cooks_d))

View(s_dat_cooksoutliers_dropped)

mod_s.cook <- rma.mv(yi, vi, 
                       data = s_dat_cooksoutliers_dropped, # use dataframe without outliers here
                       level = 95,
                       method = "REML", 
                       slab = authors, 
                       tdist = TRUE, 
                       random = list(~ 1 | st_id, 
                                     ~ 1 | eff_id))
summary(mod_s.cook)
convert_z2r(mod_s.cook$b)

# according to cook's distance, need to drop two of the markati et al. effect sizes
```
...
meta_anova <- function(s_dat, moderator){
...
meta_mods <- function(s_dat, moderator){
...
meta_cont <- function(s_dat, moderator){
  mods_result <- rma.mv(yi, vi,
                        data = s_dat,
                        level = 95,
                        method = "REML",
...
s_dat$sd <- sqrt(s_dat$vi)
s_dat$nsqrt <- sqrt(s_dat$sample)
s_dat$sei <- s_dat$sd/s_dat$nsqrt

multilevel.egger <- meta_anova(s_dat, s_dat$sei) # multilevel Egger test
multilevel.egger
# publication.status <- meta_anova(s_dat, s_dat$publication) #publication type, cannot evaluate bc only journal articles in here


# continuous moderators below
s_dat$mod_sample_size<-center_scale(s_dat$ssize_invert) #mean center prior to analysis
sample.size <- meta_cont(s_dat, s_dat$mod_sample_size) #sample size
sample.size
```
...
outcome1_anova <- meta_anova(s_dat, s_dat$o_measure) 
outcome1_anova
# burnout measure
outcome2_anova <- meta_anova(s_dat, s_dat$b_measure)
outcome2_anova
# ABQ dimensions
outcome3_anova <- meta_anova(s_dat, s_dat$predictor)
outcome3_anova
# design type
outcome4_anova <- meta_anova(s_dat, s_dat$design) ##the only one that does not seem interesting
outcome4_anova

# all individual moderator analyses
# measure of satisfaction
outcome1_mods <- meta_mods(s_dat, s_dat$o_measure) 
outcome1_mods
# burnout measure
outcome2_mods <- meta_mods(s_dat, s_dat$b_measure) 
outcome2_mods
# ABQ dimensions
outcome3_mods <- meta_mods(s_dat, s_dat$predictor) 
outcome3_mods
# design type
outcome4_mods <- meta_mods(s_dat, s_dat$design) 
outcome4_mods
...
sig_mods_anova <- rma.mv(yi, vi,
                         ...
                         method = "REML",
                         tdist = TRUE,
                         mods = ~s_dat$predictor, #add significant moderators here from meta_anova above
                         random = list(~ 1 | st_id, 
                                       ~ 1 | eff_id))
sig_mods_anova

# model comparisons: maximum-likelihood models (rather than REML) are required
mod_s_ml <- rma.mv(yi, vi, 
                     data = s_dat,
                     level = 95,
...
nomod_sigmod <- anova.rma(mod_s_ml, sigmod_ml) #no mods vs significant mod

# check out the comparisons
print(nomod_sigmod)
# full model is significantly better in terms of fit
...
ks <- c(35) # number of effect sizes in dataset
for (k in ks) {
  png(paste0("satisfaction forest_k=",formatC(k, format="f", flag="0", width=3, digits=0), res=1000, ".png", sep=""), 
...
pdf("satisfaction forest.pdf", width = 17, height = 15, pointsize = 10)
k <- 35
forest(mod_s,
       order ="obs", # order by effect size
       level = 95,
...
cols <- palette.colors(length(unique(s_dat$st_id)), palette="Alphabet") # use different colours to visualise effects from same study
cols <- cols[as.numeric(factor(s_dat$st_id))]
...
pdf("funnel-satisfaction.pdf", width = 11.2, height = 6.4, pointsize = 10) #pdf (preferred format)
funnel(mod_s,
...
pdf("sunset funnel-satisfaction.pdf", width = 11.2, height = 6.4, pointsize = 10) 
viz_sunset(mod_s, 
           text_size = 5,
           point_size = 3,
           true_effect = -.20) # true effect that should be assumed from power calculations
dev.off() # execuites plots
...
pdf("contour enhanced funnel-satisfaction.pdf", width = 11.2, height = 6.4, pointsize = 10) 
viz_funnel(mod_s,
           trim_and_fill = T,
